        ...
        <div class="box_navbar fixed-top shadow shadow-sm">
            <div class="container d-flex justify-content-between">
                <div class="d-flex gap-1">
                    <?php
                    $db = db('menu');
                    $q1[] = ['id' => 0, 'no_urut' => 0, 'role' => session('role'), 'menu' => 'Home', 'tabel' => 'users', 'controller' => 'home', 'icon' => "fa-solid fa-earth-asia", 'url' => 'home', 'logo' => 'file_not_found.jpg', 'grup' => ''];
                ...
                <div class="pt-1">
                    <a href="" class="navbar_link lonceng_notif"><i class="fa-solid fa-bell"></i> <span class="jml_notif">0</span></a>
                    <a href="" class="navbar_link"><?= session('nama'); ?>/<?= session('role'); ?></a>
                    <a class="navbar_link_danger" href="<?= base_url('logout'); ?>"><i class="fa-solid fa-arrow-right-to-bracket"></i> Logout</a>
                </div>
            </div>
        </div>
            ...
            <div class="container d-flex justify-content-between">
                <div class="d-flex gap-1">
                    <?php foreach (menus() as $i) : ?>
                        ...
                        <a href="<?= base_url($i['controller']); ?>" class="navbar_link <?= (url() == $i['controller'] ? 'navbar_active' : ''); ?>"><i class="<?= $i['icon']; ?>"></i><?= ($i['menu'] !== 'Home' ? ' ' . upper_first($i['url']) . ' ' : ''); ?><?= $i['menu']; ?></a>
                    <?php endforeach; ?>
...
<div class="d-block d-md-none d-sm-block fixed-top" style="top:-5px">
    <div class="container bg_1 py-2 shadow shadow-sm">
        <div class="d-flex justify-content-between">
            ...
            <div class="d-flex justify-content-center gap-1">
                <div class="pt-1">
                    <span class="px-2 pb-1 bg_2 rounded" style="font-size:small"><?= session('nama'); ?></span>
                </div>
                <div class="pt-1">
                    <span class="px-2 pb-1 bg_2 rounded" style="font-size:small"><i class="<?= menu()['icon']; ?>"></i> <?= menu()['menu']; ?></span>
...
<div class="offcanvas offcanvas-start" style="width:90%" data-bs-scroll="true" tabindex="-1" id="leftMenu" aria-labelledby="leftMenuLabel">
    <div class="offcanvas-header shadow shadow-bottom shadow-sm bg_1">
        <h6 class="offcanvas-title" id="leftMenuLabel">Menu</h6>
        <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body bg_2">
        <?php foreach (menus() as $i) : ?>
            <div class="mb-1 d-grid">
                <a href="<?= base_url($i['controller']); ?>" style="font-size: small;" class="rounded px-3 py-1 <?= (url() == $i['controller'] ? 'link_3' : 'link_4'); ?>"><i class="<?= $i['icon']; ?>"></i> <?= $i['menu']; ?></a>
            </div>
        <?php endforeach; ?>
        <div class="d-grid">
            <a class="navbar_link_danger" href="<?= base_url('logout'); ?>"><i class="fa-solid fa-arrow-right-to-bracket"></i> Logout</a>
